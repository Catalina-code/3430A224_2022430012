...
public:
    ...
    void retirarContenedor() {
        ...
        if (!espacioSuficiente) {
            // Volver a colocar los contenedores que se retiraron
            for (auto& cont : moverContenedores) {
                bool colocacionExitosa = false;
                for (auto& pila : pilas) {
                    if (pila.agregarContenedor(cont)) {
                ...
                if (!colocacionExitosa) {
                    cout << "No se pudo devolver los contenedores a las pilas. Revise el estado actual de las pilas." << endl;
                    mostrarPrimerosContenedores();
                    return;
                }
            }
            cout << "No se pudo retirar el contenedor " << numero << " debido a la falta de espacio en otras pilas." << endl;
            mostrarPrimerosContenedores();
            return;
        }
